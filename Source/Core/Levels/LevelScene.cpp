...
void buildLayerEntities(World& world, const TileMap& map, std::int32_t order,
...
void buildLevelScene(World& world, const Level& level,
    ...
    if (level.layers().empty()) {
        buildLayerEntities(world, level.tileMap(), 0, regionForTile, onTileEntity);
        return;
    }
    ...
    for (const TileLayer& layer : level.layers()) {
        ...
        buildLayerEntities(world, layer.tiles, order, regionForTile, onTileEntity);
        ++order;
    }
}
