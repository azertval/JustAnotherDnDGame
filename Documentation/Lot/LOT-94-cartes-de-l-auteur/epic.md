# LOT-94 — Les images du corpus quittent le dépôt ; l'écran « Carte » revient sur les cartes de l'auteur : monde, région, ville {#lot-94}

> Statut : **livré le 17 septembre 2026** (ajouté à la feuille de route le 16 ; PR #65). Vérification : voir
> *Acceptation, telle que vérifiée* plus bas.
> Prérequis : [LOT-87](@ref lot-87) (le cahier des assets, où la pièce `ui/background/menu-scene`
> est décrite, et la maquette 08 de l'écran de carte), [LOT-39](@ref lot-39) (le manifeste de
> provenance, `illustrations.json`), [LOT-37](@ref lot-37) (l'atlas, dont les lieux se placent sur
> les cartes).
> Alimente : [LOT-28](@ref lot-28), `LOT-42`, `LOT-96`.
> Absorbe : `LOT-95` (le plan de la Capitale), retiré par fusion le 17 septembre 2026.
> Exigences couvertes : `EX-IHM-076` (refondue), `EX-IHM-106`, `EX-IHM-107` (détail plus bas).
...
## Acceptation, telle que vérifiée
...
- L'écran « Carte » s'ouvre depuis le cadre de jeu, descend du monde à une région puis à une ville
  et remonte, au clavier, à la manette et à la souris ; une capture de référence QML par niveau.
- `check_map_assets.py` est vert : « 16 carte(s) conforme(s) au manifeste, à l'atlas et à
  world-maps.json ».
- `test_world_maps.cpp` couvre la lecture et la jointure ; `test_rpg_screens.cpp` compte neuf
  écrans.
- `lint_lots.py` et `lint_exigences.py` verts ; le `LOT-42`, le `LOT-96`, le
  [LOT-27](@ref lot-27) et le [LOT-28](@ref lot-28) citent ce lot.
