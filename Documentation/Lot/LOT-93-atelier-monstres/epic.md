...
- Le lion, le loup et le soldat Ironhand ont, dans `Source/Elements/Assets/Monsters/<slug>/`, leurs
  bandes aux dimensions de leur gabarit, leurs `.anim.json` et `portrait.png`, et une entrée au
  manifeste — sans `cast`.
- Ils se jouent dans la galerie des assets, animation par animation, et passent la revue.
- `check_asset_keys.py` rend 0.
- Leurs références sont dans `atelier/monstres/<slug>/` (`fiche.json`, `prompt_b.txt`,
  `portrait.png`, `palette.txt`) ; la planche du lion est versée dans `atelier/ancres/planche_grand.png`.
