...
> 1130/1130 en Debug et en Release, `clang-format`, les lints, le contrôle des données RPG et le
> cahier de test verts ; deux défauts réintroduits à la main font chacun échouer leur test).
> Prérequis : [LOT-19](@ref lot-19) (la grille), [LOT-21](@ref lot-21) (l'attaque et son jet
> amendable).
> Alimente [LOT-23](@ref lot-23), [LOT-24](@ref lot-24), [LOT-25](@ref lot-25), `LOT-35` et
> `LOT-72`.
> Exigences couvertes : `EX-CBT-021` (la ligne de vue et la couverture), `EX-CBT-022` (l'arme
> déclare son allonge ou sa portée). Aucune exigence ajoutée.
...
- **Symétrie de la ligne de vue vérifiée exhaustivement sur des grilles générées.** ✔ Vingt grilles,
  chaque paire de points de grille et d'emprises, dans les deux sens ; un tracé asymétrique
  réintroduit fait échouer le test.
- **Les gabarits couvrent exactement les cases attendues, figées par des cas de référence.** ✔
  Sphère, cylindre, cône, ligne cardinale et diagonale, cube, comparés à leur dessin case pour case ;
  l'origine incluse ou non ; une zone coupée par un mur.
- **La couverture modifie la CA du montant prévu, et ne s'applique jamais deux fois.** ✔ +2, +5, un
  abri reposé sans effet, un meilleur qui remplace ; un bonus cumulé réintroduit fait échouer le
  test.
- **Testable headless.** ✔ Tout est dans `Core`, et se teste sans fenêtre.
- `ctest` : **1130/1130** (1120 avant ; trois tests de ligne de vue, trois de zones, trois
  d'attaque, un d'arène). ✔
