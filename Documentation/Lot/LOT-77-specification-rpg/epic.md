...
> Statut : **fait** (vérification automatisée : `scripts/lint_exigences.py` vert sans exception
> ajoutée, `scripts/lint_lots.py` vert, Doxygen vert).
> Prérequis : aucun. Prérequis de toute la filière contenu, à commencer par le `LOT-30`.
...
- `python scripts/lint_exigences.py` passe au vert **sans entrée ajoutée** à
  `UNREFERENCED_ALLOWED` — 339 exigences déclarées, 339 référencées. ✔
