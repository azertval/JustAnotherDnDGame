...
> 1120/1120 en Debug et en Release, `clang-format`, les lints, la Doxygen et le cahier de test
> verts ; deux défauts
> réintroduits à la main font chacun échouer leurs tests ; écran de l'arène capturé).
> Prérequis : [LOT-13](@ref lot-13) (la fiche), [LOT-14](@ref lot-14) (l'arme portée),
> [LOT-20](@ref lot-20) (le tour et ses crochets).
> Alimente [LOT-22](@ref lot-22), [LOT-23](@ref lot-23), [LOT-24](@ref lot-24), `LOT-25`,
...
- **Un 20 naturel double les dés et non le modificateur, vérifié par test.** ✔ Sur deux cents
  graines, 1d6+3 critique tient entre 5 et 15 ; la même règle se vérifie dans une attaque résolue.
- **Un 1 naturel rate, même avec un total supérieur à la CA.** ✔ 31 contre CA 5 : raté.
- **PV bornés à 0.** ✔ Aux trois bornes — un PV de moins que le total, le total, au-delà — avec la
  chute annoncée une fois et l'excédent rapporté.
- **Inconscience et jets de sauvegarde contre la mort testés aux bornes.** ✘ Au `LOT-72`, où la
  feuille de route les a fusionnés : ce critère est antérieur à la fusion. Ce que l'agonie lira est
  livré et testé.
- **Résistances, vulnérabilités et immunités appliquées dans le bon ordre.** ✔ L'exemple du Manuel,
  résistant et vulnérable, deux résistances, immunité, contournement, source qui ignore les
  résistances.
- **Chaque jet produit une entrée de journal complète et lisible.** ✔ Comparée mot pour mot, touchée
  et ratée.
- **Deux défauts réintroduits à la main** — doubler le modificateur d'un critique, appliquer la
  vulnérabilité avant la résistance — font échouer, le premier deux tests, le second le test d'ordre.
- `ctest` : **1120/1120** (1107 avant ; sept tests de dégâts, six d'attaque, un d'arène pour
  l'opportunité ; le test des kits du coup d'essai est retiré, celui du coup est réécrit). ✔
