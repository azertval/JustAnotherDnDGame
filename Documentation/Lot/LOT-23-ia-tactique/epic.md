...
> 1139/1139 en Debug, les lints et le contrôle des données RPG verts ; deux défauts
> réintroduits à la main font chacun échouer leur test).
> Prérequis : [LOT-20](@ref lot-20) (le tour), [LOT-21](@ref lot-21) (l'attaque),
> [LOT-22](@ref lot-22) (la portée, la vue et l'abri).
> Alimente [LOT-24](@ref lot-24), [LOT-27](@ref lot-27), `LOT-46`.
> Exigences couvertes : `EX-CBT-050` (l'IA choisit dans les mêmes actions que le joueur, avec les
> mêmes informations). Aucune exigence ajoutée.
...
- **Un combat IA contre IA se termine toujours, sur un échantillon de configurations générées.** ✔
  Trente salles, compositions et profils générés ; chaque combat a son issue.
- **Rejeu à graine fixe strictement reproductible.** ✔ Même journal et même nombre de tours au
  rejeu ; aucune décision ne passe par un flottant.
- **L'IA ne finit pas son tour à portée de trois ennemis quand une position sûre existait.** ✔ Pour
  les cinq profils, sur la case la plus rentable et la seule dangereuse ; le défaut réintroduit fait
  échouer le test.
- **Tests headless intégralement automatisables.** ✔ Tout est dans `Core`, joué sans fenêtre.
