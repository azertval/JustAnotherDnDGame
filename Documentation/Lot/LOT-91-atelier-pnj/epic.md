# LOT-91 — Atelier des PNJ : les figurines du Character Compendium {#lot-91}
...
> au fil de l'eau, au rythme du budget de génération.
> Prérequis : [LOT-50](@ref lot-50) (le Colisée, ses bandes 48 × 64, son pilote d'animation),
> [LOT-39](@ref lot-39) (familles d'assets, marqueur).
...
> chaque tour, chaque défaut et chaque décision ; cet epic n'en garde que la méthode.
...
- Chaque PNJ coché a, dans `Source/Elements/Assets/Npc/<slug>/`, ses six bandes aux dimensions du jeu
  d'animations, leurs `.anim.json` et `portrait.png`, et figure dans `Npc/manifest.json`.
- Chaque PNJ coché a ses références dans `atelier/pnj/<slug>/` (`prompt_b.txt`, `portrait.png`,
  `palette.txt`).
