...
> 1065/1065 en Debug et en Release, `clang-format`, `clang-tidy` `bugprone-*` sur les deux
> fichiers ajoutés, les lints, cahier de test et Doxygen verts).
...
- **Même entrée → même chemin, systématiquement.** ✔ Cent requêtes identiques rendent le même
  chemin ; la règle de départage est vérifiée sur deux cas écrits à la main ; `findPath` et
  `pathTo` s'accordent sur toute destination de deux cents cartes aléatoires à graine fixe.
- **L'ensemble des cases atteignables correspond exactement au budget**, ni une de plus ni une de
  moins. ✔ Budgets 0 à 5 sur une grille ouverte, chaque case comparée à sa distance.
- **Deux créatures ne partagent jamais une case.** ✔ Y compris par l'emprise d'une grande créature.
- **Le terrain difficile double le coût, et la portée s'en trouve réduite en conséquence.** ✔ La
  boue partout ramène 80 destinations à 24, et une case difficile à un pas du bout du budget reste
  hors de portée.
