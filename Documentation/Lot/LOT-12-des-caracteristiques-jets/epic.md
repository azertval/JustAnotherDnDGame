...
- Distribution vérifiée sur **100 000 tirages** à graine fixe : les vingt faces sortent, aucune ne
  s'écarte de plus de 10 % de la moyenne. ✔
- **Rejouabilité stricte** : même graine → même séquence, dé par dé ; une graine différente donne
  une séquence différente. ✔
- Avantage = max de deux d20, désavantage = min ; les deux ensemble **s'annulent**, y compris à 2
  contre 1. ✔
- 1 et 20 naturels détectés et **distingués** d'un total de 1 ou 20. ✔
- Aucune dépendance ECS, Qt ou rendu : `Core/Rpg/` compile seul. ✔
- Une notation mal formée est refusée — dont `ld8`, la faute d'OCR que le
  [LOT-30](@ref lot-30) a documentée. ✔
