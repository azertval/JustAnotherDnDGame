...
- L'exécutable **historique** `JustAnotherRpgGame` (`main.cpp`, `hmi::Window`, `ScreenManager`, `…Screen`)
...
- La cible Qt `JustAnotherRpgGame` est déclarée ici (vide/minimale à cette tâche : un `main` Qt qui
...
- La release produit un exe **autonome `/MTd`**. Qt (LGPL) impose le **lien dynamique** : ajouter une
  étape **`windeployqt`** (`windeployqt build\vs\bin\Debug\JustAnotherRpgGame.exe`) pour copier les
...
- (Le retrait de l'ancien exe et le passage de la release sur `JustAnotherRpgGame` se finalisent au
...
- **Non-régression build** : `cmake --preset vs` + `cmake --build --preset vs` compilent `Core`,
  `HmiRuntime`, `JustAnotherRpgGame` (legacy), `JustAnotherRpgGame` (nu) et `UnitTests`.
...
- `HMI` scindé en bibliothèque `HmiRuntime` + exe legacy fonctionnel ; cible Qt `JustAnotherRpgGame`
