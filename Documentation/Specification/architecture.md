...
- \anchor EX-ARCH-040 **EX-ARCH-040** — `Core` définit **ses propres types** mathématiques (`Vector2`, `Rect`, …), **sans dépendance graphique**. La conversion vers `DirectXMath` a lieu uniquement à la frontière de rendu (`HMI`).

## 6. Abstraction de rendu
- \anchor EX-ARCH-050 **EX-ARCH-050** — Le rendu est un **pipeline mince** (lot de sprites) au-dessus de **QRhi**, qui retient Direct3D 11 sous Windows (`EX-REN-002`). Le projet n'ajoute aucune couche d'abstraction graphique propre.
...
- \anchor EX-ARCH-002 **EX-ARCH-002** *(retirée au `LOT-88`)* — ordre des passes d'un pas fixe
  porté par un mode de jeu : l'orchestrateur à passes est retiré ; chaque session — exploration,
  arène — porte son propre ordre.
- \anchor EX-ARCH-031 **EX-ARCH-031** *(retirée au `LOT-88`)* — facteur d'interpolation fourni au
  rendu : aucun rendu n'interpole plus entre deux pas.
- \anchor EX-ARCH-090 **EX-ARCH-090** *(retirée au `LOT-88`)* — état « Éditeur » parmi les états du
  jeu : l'éditeur est un exécutable distinct (`EX-EDIT-030`).
- \anchor EX-ARCH-100 **EX-ARCH-100** *(retirée au `LOT-88`)* — décors en entités de la simulation.
