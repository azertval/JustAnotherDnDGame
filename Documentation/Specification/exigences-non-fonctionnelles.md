...
## 1. Performance
- \anchor EX-NFR-001 **EX-NFR-001** — Le jeu doit maintenir **60 images/seconde** sur une
...
- \anchor EX-NFR-003 **EX-NFR-003** — L'empreinte mémoire doit rester stable dans le temps (aucune
...
- \anchor EX-NFR-022 **EX-NFR-022** — La **CI** doit exécuter build, tests et couverture à chaque
...
- \anchor EX-NFR-032 **EX-NFR-032** — Cible actuelle : **Windows/Direct3D**. Le `Core` doit toutefois éviter toute dépendance système inutile pour préserver sa testabilité (et une portabilité future).

## 5. Robustesse
- \anchor EX-NFR-040 **EX-NFR-040** — Une erreur récupérable (fichier de carte invalide, ressource manquante) ne doit pas faire planter le jeu : elle est signalée et gérée (cf. politique d'erreurs des conventions).
- \anchor EX-NFR-041 **EX-NFR-041** — Les ressources (mémoire, ressources GPU) doivent être gérées en **RAII** (libération garantie).
...
- \anchor EX-NFR-021 **EX-NFR-021** *(retirée en `LOT-67`)* — test système de franchissabilité des
  niveaux : un bac à sable n'a pas de sortie qui le termine.
- \anchor EX-NFR-043 **EX-NFR-043** *(retirée au `LOT-88`)* — plafond de mémoire de texture des
  plans picturaux.
