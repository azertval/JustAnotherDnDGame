...
> `lot-editor-03-pieces`. Vérification automatisée : construction `/W4 /WX` sans avertissement,
> tests unitaires verts (813), acceptation sur Martpart, `LevelEditor --check` vert ; capture de la
> fenêtre (palette des pièces, masque des cases forcées). **Vérification à la souris due** : poser,
> glisser et gommer dans le canevas (voir plus bas).
> Prérequis : `LOT-EDITOR-02`, `LOT-EDITOR-12`. Feuille de route : [éditeur](@ref roadmap-editeur).
> Alimente : `LOT-EDITOR-04`.
