...
> `lot-editor-04-outils`. Vérification automatisée : construction `/W4 /WX` sans avertissement,
> tests unitaires verts (826, dont deux ignorés comme avant), acceptation sur Martpart ; capture de
> la fenêtre (barre d'outils à dix outils et miroir, menu Tools). **Vérification au clavier et à la
> souris due** : les touches et les clics postés n'atteignent pas Qt (voir plus bas).
> Prérequis : `LOT-EDITOR-03`. Feuille de route : [éditeur](@ref roadmap-editeur).
> Alimente : `LOT-EDITOR-13` (sans fenêtre), `LOT-EDITOR-08` (tampons).
