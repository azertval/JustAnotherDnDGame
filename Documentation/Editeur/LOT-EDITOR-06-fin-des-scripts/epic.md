...
> rendus relus à l'œil. **Vérification à la souris due** (voir plus bas).
> Prérequis : `LOT-EDITOR-05`, `LOT-EDITOR-13`. Feuille de route : [éditeur](@ref roadmap-editeur).
> Alimente : `LOT-EDITOR-07` (contrôles), `LOT-EDITOR-14` (renommer, remplacer), et les cartes du
> [LOT-27](@ref lot-27).
> Exigences : `EX-EDIT-077`, `EX-EDIT-078` (nouvelles). Guide : @subpage guide-usage-editeur.
...
- **Les retouches se font par `--apply`**, avec les fonctions mêmes que la souris
  (`LOT-EDITOR-13`), et leurs fichiers de gestes sont versionnés dans ce dossier (`retouches/`) :
  elles se relisent, se rejouent, et la CI en publie le rendu. La main de l'auteur ne se simule
  pas ; la vérification à la souris reste due.
...
- **Une nouvelle carte choisit son lieu** (`EX-EDIT-077`) : « New » demande un nom, une taille et
  ...
  reste une grille unique. `core::LevelDraft::setLayerProperty` pose la propriété, en un pas d'annulation. Changer le lieu
  d'une carte existante reste au `LOT-EDITOR-14` (il faut une table de correspondance des pièces).
