...
Usage :
    python scripts/check_map_assets.py            # code de sortie non nul si divergence
    python scripts/check_map_assets.py --write    # reecrit tailles et empreintes du manifeste
"""
        ...
        for field in ("size", "bytes", "sha256"):
            if actual[field] != entry.get(field):
                fail(
                    ...
                    f"    python scripts/check_map_assets.py --write"
                )
                break
