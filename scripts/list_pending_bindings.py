...
fichier de cablage de l'ecran (`Source/Ui/Screens/<Ecran>.qml`) et retire `pending: true`. Le
FORMULAIRE (`<Ecran>Form.ui.qml`) n'a pas a bouger : la mise en page decidee aujourd'hui est
conservee telle quelle. C'est precisement ce que la separation achete.

Usage :
    python scripts/list_pending_bindings.py            # inventaire lisible
    python scripts/list_pending_bindings.py --keys     # une cle par ligne (pour un grep)
"""
...
def main() -> int:
    ...
    if not UI.is_dir():
        print(f"Repertoire introuvable : {UI}", file=sys.stderr)
        return 1
    ...
    if not screens:
        ...
        print("Aucune cle en attente sous Source/Ui.")
        print("  Soit tous les ecrans sont branches, soit ce releve ne lit plus rien :")
        print(f"  verifier {UI.relative_to(ROOT).as_posix()}.")
        return 0
